...
(define (message->codes message)
  (map (λ(x) (if (eq? x #\space) 0 (- (char->integer x) 96))) (string->list message))
  )
...
(define (codes->message codes)
  (list->string (map (λ(x) (if (= x 0 ) #\space (integer->char (+ x 96)))) codes))
  )
...
(define (extend-key key size)
  (let ((l (length key)))
    (if (< l size)
      (extend-key (append key key) size)
      (take key size)
      )
    )
  )
...
(define process-codes
  (λ(op)
    (λ(y key)
      (map (λ(w v) (modulo (op w v) 27)) y (extend-key key (length y)))
      )
    )
  )
