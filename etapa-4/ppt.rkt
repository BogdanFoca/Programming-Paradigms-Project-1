...
(define (pairs G H)
  (let loop ((acc '()) (g G) (h H))
    (if (null? acc)
        (if (< (stream-first g) (stream-first h)) (loop (cons (stream-first g) (stream-first h)) (stream-rest g) h) (loop '() G (stream-rest h)))
        (stream-cons acc (if (< (stream-first g) (stream-first h)) (loop (cons (stream-first g) (stream-first h)) (stream-rest g) h) (loop '() G (stream-rest h))))
        )
    )
  )
...
(define gh-pairs-stream
  (stream-filter (λ(pair) (= 1 (gcd (car pair) (cdr pair)))) (pairs (stream-filter odd? naturals) (stream-rest (stream-filter odd? naturals))))
  )
...
(define ppt-stream-in-pair-order
  (stream-map (λ(pair) (let ((g (car pair)) (h (cdr pair))) (list (* g h) (/ (- (expt h 2) (expt g 2)) 2) (/ (+ (expt h 2) (expt g 2)) 2)))) gh-pairs-stream)
  )
